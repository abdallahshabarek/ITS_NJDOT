TOD_phase_optimization_from_historical_dates <- function(site,dates_list,analysis_time_type){
  ...
  # site <- 10019
  # df <- readr::read_csv("https://raw.githubusercontent.com/abdallahshabarek/ITS_NJDOT/main/History_Viewer/History_Viewer_Data_Archieve/Rt1_Phases_2021-12-01.csv")
  # df <- df[df$Site == site,]
  ...
  for (p_k in dates_list){
    ...
    df <- readr::read_csv(paste0("https://raw.githubusercontent.com/abdallahshabarek/ITS_NJDOT/main/History_Viewer/History_Viewer_Data_Archieve/",Rt_code,"_Phases_",p_k,".csv"))
    df <- df[df$Site == site,]
    ...
    # df_final_phases$time <- as.integer(df_final_phases$time/60)
  }
