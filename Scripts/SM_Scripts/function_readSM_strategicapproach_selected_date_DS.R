...
on a.phase = b.phase_number", drv ="SQLite")
    ...
    df22 <- df22 %>% dplyr::filter(subsite == Main_site & strategic_approach %in% strategic_approach_list)
    df22 <- df22[!duplicated(df22), ]
    
    df22 <- sqldf::sqldf("select a.subsite, a.time, 
    avg(a.Cycle_Length) as Cycle_Length, 
    avg(a.Required_Cycle_Length) as Required_Cycle_Length,
    avg(a.Avg_DS_Phase) as Avg_DS_Phase from df22 as a
    group by a.subsite,  a.time
    order by cast(a.time as int) asc", drv = "SQLite")
    ...
    df222 <- sqldf::sqldf("select b.subsite, a.time, b.Cycle_Length, b.Required_Cycle_Length, b.Avg_DS_Phase  from df_time as a
left join df22 as b
on a.time = b.time",drv="SQLite")
    ...
    df222 <- df222 %>% tidyr::fill(Avg_DS_Phase, .direction = "down")
    df222 <- df222 %>% tidyr::fill(Avg_DS_Phase, .direction = "up")
    
    
    df222_final <- rbind(df222_final, df222)

    }
  }
  df222_final <- sqldf::sqldf("select a.subsite, a.time, 
    avg(a.Cycle_Length) as Cycle_Length, 
    avg(a.Required_Cycle_Length) as Required_Cycle_Length,
    avg(a.Avg_DS_Phase) as Avg_DS_Phase 
